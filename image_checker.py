...
def compare_images(img1, img2, method=cv2.TM_CCOEFF_NORMED, grayscale=True):
    """
    Compares the similarity between two images using template matching.
    ...
    Returns:
        Similarity score (float). Higher means more similar for most methods.
    """
    if grayscale:
    ...
    res = cv2.matchTemplate(img1_resized, img2_resized, method)
    ...
    if method in [cv2.TM_SQDIFF, cv2.TM_SQDIFF_NORMED]:
        similarity = 1 - min_val  # Invert so higher is better
    else:
        similarity = max_val
